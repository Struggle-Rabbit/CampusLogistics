...
type RepairOrderListByPageReq struct {
	...
	RepairType int     `form:"repair_type"` // 报修类型：1-水电 2-家具 3-网络 4-其他
	Contact    string  `form:"contact"`     // 联系人
	Phone      string  `form:"phone"`       // 联系电话
	Status     int     `form:"status"`
	HandlerID  *string `form:"handler_id"`
	StartTime  string  `form:"start_time"` // 开始时间 (2006-01-02 15:04:05)
	EndTime    string  `form:"end_time"`   // 结束时间 (2006-01-02 15:04:05)
}
...
type RepairOrderResult struct {
	...
	Records     []*RepairRecordResult `json:"records,omitempty"` // 流转记录
}

type RepairRecordResult struct {
	ID         string    `json:"id"`
